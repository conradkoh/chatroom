...
- task/native delivery continues to react to relevant state changes;
...
1. **Separate data model per daemon use case**
   ...
   Every materially different daemon use case has its own underlying data model and Convex subscription. At minimum, agent operational status, machine status/connectivity, task delivery state, stop/restart lifecycle, and role removal must not share one broad invalidation table merely for implementation convenience.

   Validation:

   - [ ] Each daemon use case is mapped to exactly one primary signal table or an explicitly documented set of atomic tables.
   - [ ] Each use case has a separate Convex query/subscription endpoint.
...
3. **No daemon polling except heartbeat**
   ...
   The daemon must use reactive Convex subscriptions or explicit one-shot reads for all non-heartbeat behavior. Periodic polling, refresh loops, and repeated query calls intended to detect changes are prohibited.

   Validation:

   - [ ] The only recurring daemon polling loop is the daemon heartbeat.
...
hydrate chatroom_agentRoleOperationalStatus
        ↓
apply local read model + process affected task snapshots
```
...
- Writer helper: `src/domain/usecase/agent/write-machine-operational-signal.ts`
...
agent stop/restart lifecycle signals
        → stop/restart inbox
...
- [ ] Extract shared inbox infrastructure from `operational-inbox.ts`:
  ...
  - observability hooks.
- [ ] Implement one inbox per new signal table/use case.
...
### 3. Delete the daemon inbox for `subscribeMachineOperationalSignalsSince`
