...
dedup marker (`agentTaskState`) are in-memory `Map`s. The backend's status
...
| M5  | `chatroom handoff` CLI → `messages.handoff` → Step-1 `collectActiveTasks`/`completeTasks` + `terminalHandoffKey` dedup → creates next task for target role                 | CLI handoff → backend records handoff message (unchanged) → daemon task service completes the active task exactly once and creates/adopts the next task via inbox feed | V10, V14 |
