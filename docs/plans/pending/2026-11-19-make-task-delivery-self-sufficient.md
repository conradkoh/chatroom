...
Ordering note: rows 9–10 depend on the additive side (daemon agent-model read
model from launch requests) landing first.
...
| --- | --------------------------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 14  | backend · agent config inbox            | `chatroomWorkspaceAgentConfigInbox` table (durable, pending → processed) | When a launch request is recorded or superseded, a pending config event is written per `(machineId, chatroomId, role)` **in the same mutation** that writes task inbox events; payload is the agent config — `agentHarness`, `model`, `workingDir`; same lifecycle as `chatroomWorkspaceTaskInbox` | ⬜   |
...
- Coverage: inbox events are only ever written to `(machineId, role)` targets
  ...
  (`listLastSentLaunchRequestsForChatroom`), and the config event is written in
  the same mutation — a task event is never processed before its config event.
  The ephemeral duplication in task events buys nothing.
