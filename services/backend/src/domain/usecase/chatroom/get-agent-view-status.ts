...
import { deriveAgentRoleViewState, type OperationalState } from '../agent/derive-agent-operational-state';
...
async function getAgentViewStatusLegacy(ctx: QueryCtx, input: { chatroomId: Id<'chatroom_rooms'>; userId: Id<'users'> }): Promise<AgentViewStatus | null> {
  ...
  const machines = machineIds.length ? await ctx.db.query('chatroom_machines').withIndex('by_userId', (q) => q.eq('userId', input.userId)).collect() : [];
  const machineNames = new Map(machines.filter((m) => machineIds.includes(m.machineId)).map((m) => [m.machineId, m.hostname]));
  const firstUserMessage = await ctx.db.query('chatroom_messages').withIndex('by_chatroom_senderRole_type_createdAt', (q) => q.eq('chatroomId', input.chatroomId).eq('senderRole', 'user').eq('type', 'message')).first();
