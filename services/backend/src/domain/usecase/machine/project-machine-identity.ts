...
export async function upsertMachineIdentity(
  ctx: MutationCtx,
  input: { machineId: string; userId: Id<'users'>; hostname: string }
): Promise<void> {
  ...
  if (existing)
    await ctx.db.patch('chatroom_machineIdentity', existing._id, {
      userId: input.userId,
