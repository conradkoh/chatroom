...
export async function upsertMachineTaskStatusSignalHead(
  ...
  await ctx.db.patch('chatroom_machineTaskStatusSignalHeads', existing._id, {
    previousSignalKey: existing.latestSignal.signalKey,
    latestSignal: signal,
  });
}

export async function deleteMachineTaskStatusSignalHead(
  ctx: MutationCtx,
  machineId: string
): Promise<void> {
  const existing = await ctx.db
