...
export type AuthorizeAgentStartArgs = { chatroomId: Id<'chatroom_rooms'>; role: string; machineId: string; lifecycleRevision?: number; taskId?: Id<'chatroom_tasks'> };
export type AuthorizeAgentStartReason = 'stale_revision' | 'stopped' | 'disabled' | 'stop_in_flight' | 'not_configured' | 'no_active_task';
...
export async function authorizeAgentStart(ctx: MutationCtx, args: AuthorizeAgentStartArgs): Promise<AuthorizeAgentStartResult> {
  ...
  if (isEphemeralAgentRole(args.role)) {
    ...
    if (!task || task.chatroomId !== args.chatroomId || task.assignedTo?.toLowerCase() !== args.role.toLowerCase() || !['pending', 'acknowledged', 'in_progress'].includes(task.status)) return { allowed: false, reason: 'no_active_task' };
  }
