...
export async function rollupAgentStopCommandStatus(ctx: MutationCtx, stopCommandId: Id<'chatroom_agentStopCommands'>): Promise<void> {
  ...
  if (!command || command.status === 'completed' || command.status === 'failed' || command.status === 'superseded') return;
  ...
  const terminal = (status: string) => status === 'completed' || status === 'failed' || status === 'superseded';
  ...
  if (!failed && command.postStopDesiredState) {
    ...
    if (room?.teamId) for (const role of [...new Set(targets.map((target) => target.role))]) {
      const config = await ctx.db.query('chatroom_teamAgentConfigs').withIndex('by_teamRoleKey', (q) => q.eq('teamRoleKey', buildTeamRoleKey(command.chatroomId, room.teamId!, role))).first();
      if (config && config.desiredState !== command.postStopDesiredState) await patchTeamAgentConfig(ctx, config._id, { desiredState: command.postStopDesiredState }, { projectScope: 'chatroom' });
    }
  }
