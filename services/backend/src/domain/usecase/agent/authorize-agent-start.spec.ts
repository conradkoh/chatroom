...
describe('authorizeAgentStart', () => {
  test('allows matching revision and rejects stale, stopped, disabled, and wrong machine', async () => {
    ...
    expect(await t.run((ctx) => authorizeAgentStart(ctx, { chatroomId, role: 'builder', machineId, lifecycleRevision: 0 }))).toEqual({ allowed: true, lifecycleRevision: 0 });
    const config = await configId(chatroomId);
    await t.run((ctx) => ctx.db.patch(config!._id, { lifecycleRevision: 2 }));
    expect(await t.run((ctx) => authorizeAgentStart(ctx, { chatroomId, role: 'builder', machineId, lifecycleRevision: 0 }))).toEqual({ allowed: false, reason: 'stale_revision' });
    await t.run((ctx) => ctx.db.patch(config!._id, { lifecycleRevision: 2, desiredState: 'stopped' }));
    expect((await t.run((ctx) => authorizeAgentStart(ctx, { chatroomId, role: 'builder', machineId }))).reason).toBe('stopped');
    await t.run((ctx) => ctx.db.patch(config!._id, { desiredState: 'running', enabled: false }));
    expect((await t.run((ctx) => authorizeAgentStart(ctx, { chatroomId, role: 'builder', machineId }))).reason).toBe('disabled');
    expect((await t.run((ctx) => authorizeAgentStart(ctx, { chatroomId, role: 'builder', machineId: 'wrong' }))).reason).toBe('not_configured');
  });
