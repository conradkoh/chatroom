...
export async function createAgentStopCommand(ctx: MutationCtx, input: CreateAgentStopCommandInput): Promise<CreateAgentStopCommandResult> {
  ...
  if (input.selectedConfigs.length === 0) await ctx.db.patch('chatroom_agentStopCommands', stopCommandId, { status: 'completed', completedAt: Date.now() });
  for (const role of [...new Set(input.selectedConfigs.map((config) => config.role))]) await projectAgentStopStateForRole(ctx, input.chatroomId, role);
