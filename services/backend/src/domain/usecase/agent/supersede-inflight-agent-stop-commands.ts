...
export async function supersedeInflightAgentStopCommands(ctx: MutationCtx, args: { chatroomId: Id<'chatroom_rooms'>; exceptStopCommandId?: Id<'chatroom_agentStopCommands'> }): Promise<Id<'chatroom_agentStopCommands'>[]> {
  ...
  const commands = await ctx.db.query('chatroom_agentStopCommands').withIndex('by_chatroom_status', (q) => q.eq('chatroomId', args.chatroomId).eq('status', status)).collect();
  for (const command of commands) {
    ...
    for (const target of targets) if (target.status === 'pending' || target.status === 'processing') await ctx.db.patch(target._id, { status: 'superseded', errorMessage: 'superseded', completedAt: Date.now() });
    ...
    for (const execution of executions) {
      if (execution.status === 'pending' || execution.status === 'processing') await ctx.db.patch(execution._id, { status: 'superseded', errorMessage: 'superseded', completedAt: Date.now() });
    ...
    await ctx.db.patch(command._id, { status: 'superseded', errorCode: 'superseded', errorMessage: 'superseded', completedAt: Date.now() });
