...
describe('rollupAgentStopCommandStatus', () => {
  ...
  test('restores postStopDesiredState on successful rollup', async () => {
    ...
    const chatroomId = await t.mutation(api.chatrooms.create, { sessionId, teamId: 'duo', teamName: 'Duo', teamRoles: ['planner', 'builder'], teamEntryPoint: 'planner' });
    const commandId = await t.run(async (ctx) => {
      const config = await ctx.db.insert('chatroom_teamAgentConfigs', { chatroomId, teamRoleKey: buildTeamRoleKey(chatroomId, 'duo', 'builder'), role: 'builder', type: 'remote', machineId: 'm', desiredState: 'stopped', enabled: true, lifecycleRevision: 0, createdAt: Date.now(), updatedAt: Date.now() });
      const id = await ctx.db.insert('chatroom_agentStopCommands', { chatroomId, scope: { kind: 'agent', role: 'builder' }, scopeKey: 'agent:builder', reason: 'platform.ephemeral_task_complete', postStopDesiredState: 'running', status: 'processing', createdAt: Date.now() });
      await ctx.db.insert('chatroom_agentStopMachineExecutions', { stopCommandId: id, chatroomId, machineId: 'm', status: 'completed', completedAt: Date.now() });
      await ctx.db.insert('chatroom_agentStopTargets', { stopCommandId: id, chatroomId, machineId: 'm', role: 'builder', pid: 1, targetKey: 'm:builder:1', revisionKey: 'r', status: 'completed', outcome: 'stopped', completedAt: Date.now() });
    ...
    const config = await t.run((ctx) => ctx.db.query('chatroom_teamAgentConfigs').withIndex('by_chatroom', (q) => q.eq('chatroomId', chatroomId)).filter((q) => q.eq(q.field('role'), 'builder')).first());
