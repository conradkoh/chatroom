...
async function setup(id: string) {
  ...
  const config = await t.run((ctx) => ctx.db.query('chatroom_teamAgentConfigs').withIndex('by_teamRoleKey', (q) => q.eq('teamRoleKey', buildTeamRoleKey(chatroomId, 'duo', 'builder'))).first());
...
describe('registerSpawnedAgentIfAuthorized', () => {
  test('accepts matching revision and records PID', async () => {
    ...
    const result = await t.run((ctx) => registerSpawnedAgentIfAuthorized(ctx, { chatroomId, role: 'builder', machineId, pid: 12345, lifecycleRevision: 0 }));
  ...
  test('rejects stale revision without changing PID', async () => {
    ...
    const result = await t.run((ctx) => registerSpawnedAgentIfAuthorized(ctx, { chatroomId, role: 'builder', machineId, pid: 999, lifecycleRevision: 0 }));
