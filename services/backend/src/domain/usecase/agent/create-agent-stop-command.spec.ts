...
describe('createAgentStopCommand', () => {
  test('supersedes inflight command per chatroom', async () => {
    ...
    const first = await t.run((ctx) => createAgentStopCommand(ctx, { chatroomId, scope: { kind: 'agent', role: 'builder' }, reason: 'user.stop', selectedConfigs: [] }));
    const second = await t.run((ctx) => createAgentStopCommand(ctx, { chatroomId, scope: { kind: 'agent', role: 'builder' }, reason: 'user.stop', selectedConfigs: [] }));
