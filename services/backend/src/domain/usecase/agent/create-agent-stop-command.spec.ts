...
describe('createAgentStopCommand', () => {
  test('supersedes inflight command per chatroom', async () => {
    ...
    const first = await t.run(async (ctx) => createAgentStopCommand(ctx, { chatroomId, scope: { kind: 'agent', role: 'builder' }, reason: 'user.stop', selectedConfigs: await selectConfigsForAgentStop(ctx, { chatroomId, scope: { kind: 'agent', role: 'builder' }, machineId }) }));
    const second = await t.run(async (ctx) => createAgentStopCommand(ctx, { chatroomId, scope: { kind: 'agent', role: 'builder' }, reason: 'user.stop', selectedConfigs: await selectConfigsForAgentStop(ctx, { chatroomId, scope: { kind: 'agent', role: 'builder' }, machineId }) }));
  ...
  test('filters role targets', async () => { const { chatroomId, machineId } = await setup('stop-command-filter'); const result = await t.run((ctx) => createAgentStopCommand(ctx, { chatroomId, scope: { kind: 'agent', role: 'planner' }, reason: 'user.stop', selectedConfigs: [] })); const targets = await t.run((ctx) => ctx.db.query('chatroom_agentStopTargets').withIndex('by_stopCommandId', (q) => q.eq('stopCommandId', result.stopCommandId)).collect()); expect(targets).toHaveLength(0); });
  test('completes zero-target command', async () => { const { chatroomId, machineId } = await setup('stop-command-empty'); const result = await t.run((ctx) => createAgentStopCommand(ctx, { chatroomId, scope: { kind: 'agent', role: 'planner' }, reason: 'user.stop', selectedConfigs: [] })); const command = await t.run((ctx) => ctx.db.get('chatroom_agentStopCommands', result.stopCommandId)); expect(command?.status).toBe('completed'); });
});
