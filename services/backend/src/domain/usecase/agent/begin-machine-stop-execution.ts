...
export async function beginMachineStopExecution(
  ...
  args: {
    ...
    machineId: string;
    inboxCommandId: Id<'chatroom_machineCommandInbox'>;
  }
) {
  ...
  if (
    (command.deadlineAt != null &&
      Date.now() > command.deadlineAt + AGENT_STOP_EXPIRY_LEASE_GRACE_MS) ||
    ['superseded', 'completed', 'failed'].includes(command.status)
  ) {
