...
export function appendEnhanceModeGuidance(lines: string[]): void {
  lines.push(
    ...
    'If specialist input is useful, you may delegate to `architect` or `uiux-engineer` as optional advisors. They are ordinary team roles; do not create a separate design task or use a special handoff ceremony.',
    '</enhance-mode>'
  );
}
