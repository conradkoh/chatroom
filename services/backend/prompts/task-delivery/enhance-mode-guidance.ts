...
export function appendEnhanceModeGuidance(lines: string[]): void {
  lines.push(
    ...
    'If architect input is useful, delegate to `architect`; if UI/UX input is useful, delegate to `uiux-engineer`. Both are ordinary team roles; do not create a separate design task or use a special handoff ceremony.',
    '</enhance-mode>'
  );
}
