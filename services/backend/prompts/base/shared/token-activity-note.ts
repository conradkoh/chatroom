...
export function getTokenActivityInProgressNote(): string {
  return 'Begin working from the task content above. The daemon detects harness output (stdout tokens) and marks the task `in_progress` automatically — **do not run `task read`** unless you need backlog or context details not shown in the delivery.';
}
