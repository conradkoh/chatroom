...
export interface GeneralKnowledgeParams { chatroomId: string; role: string; convexUrl: string; compactSkills?: boolean; nativeIntegration?: boolean }
export function getGeneralCommandsReferenceContent(params: GeneralKnowledgeParams): string {
  const cliEnvPrefix = getCliEnvPrefix(params.convexUrl); const { chatroomId, role } = params;
  const contextReadCmd = contextReadCommand({ chatroomId, role, cliEnvPrefix });
  const messagesDownloadCmd = messagesDownloadSinceCommand({ chatroomId, role, cliEnvPrefix, sinceMessageId: '<from-anchor>', limit: 100 });
...
export function getGeneralKnowledgeSections(params: GeneralKnowledgeParams, options: { includeHistory?: boolean } = {}): PromptSection[] {
  const sections: PromptSection[] = [getGlossarySection({ convexUrl: params.convexUrl, chatroomId: params.chatroomId, role: params.role, nativeIntegration: params.nativeIntegration, compactSkills: params.compactSkills })];
  if (options.includeHistory !== false) sections.push(createSection('history-retrieval', 'knowledge', getHistoryRetrievalGuidance({ chatroomId: params.chatroomId, role: params.role, cliEnvPrefix: getCliEnvPrefix(params.convexUrl) })));
  sections.push(createSection('general-commands-reference', 'knowledge', getGeneralCommandsReferenceContent(params))); return sections;
}
