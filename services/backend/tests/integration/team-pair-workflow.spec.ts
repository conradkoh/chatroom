    ...
    test('user → builder → user: direct response for questions', async () => {
      ...
      expect(builderPrompt.prompt).toMatchInlineSnapshot(`
        ...
        **Complete task and hand off:**
        ...
        \`\`\`bash
        chatroom handoff --chatroom-id="10039;chatroom_rooms" --role="builder" --next-role="<target>" << 'EOF'
        ---MESSAGE---
        [Your message here]
        EOF
        \`\`\`
        ...
        **Report progress on current task:**
        ...
        \`\`\`bash
        chatroom report-progress --chatroom-id="10039;chatroom_rooms" --role="builder" << 'EOF'
        ---MESSAGE---
        [Your progress message here]
        EOF
        \`\`\`
        ...
        **Continue receiving messages after \`handoff\`:**
        \`\`\`
        chatroom get-next-task --chatroom-id="10039;chatroom_rooms" --role="builder"
        \`\`\`
        ...
        **Re-fetch your system prompt (after context reset):**
        \`\`\`
        chatroom get-system-prompt --chatroom-id="10039;chatroom_rooms" --role="builder"
        \`\`\`"
      `);
