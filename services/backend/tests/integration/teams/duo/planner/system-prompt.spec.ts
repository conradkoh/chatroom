...
describe('Duo Team > Planner > System Prompt', () => {
  test('system prompt for custom agent', async () => {
    ...
    expect(prompt).toContain('Available targets: architect, uiux-engineer, builder, user');
    ...
    expect(prompt).toMatchInlineSnapshot(`
      ...
      **Team composition:** Duo team — you coordinate with \`architect\`, \`uiux-engineer\`, \`builder\` for implementation.
      ...
      **Before handoff to \`user\` (proof of verification):**
      1. \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom messages anchor --chatroom-id="000000000000010002chatroom_rooms" --role="planner"\` — locate the user's last message
      2. \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom messages download --chatroom-id="000000000000010002chatroom_rooms" --role="planner" --since-message-id="<from-anchor>" --limit=100\` — download grep-friendly history; read handoffs and goals
      ...
      ### Handoff Options
      Available targets: architect, uiux-engineer, builder, user
