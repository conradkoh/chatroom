...
describe('Squad Team > Planner > Wait For Task', () => {
  test('task from user', () => {
    ...
    expect(output).toMatchInlineSnapshot(`
      ...
      📋 PROCESS
      ============================================================
      1. Code changes expected? → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom context new --chatroom-id=test-chatroom-id --role=planner << 'EOF'
      ...
      EOF\`
      4. Do the work
      5. Hand off → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom handoff --chatroom-id=test-chatroom-id --role=planner --next-role=<target>\` (targets: builder, reviewer, user)
      6. Resume → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom wait-for-task --chatroom-id=test-chatroom-id --role=planner\`
      ...
      Reference commands:
        ...
        task-complete → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom task-complete --chatroom-id=test-chatroom-id --role=planner\`
      ...
      2. Code changes expected? → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom context new --chatroom-id=test-chatroom-id --role=planner << 'EOF'
      <summary of current focus>
      EOF\`
