...
describe('Squad Team > Builder > Get Next Task', () => {
  test('task from user', () => {
    ...
    expect(output).toMatchInlineSnapshot(`
      ...
      EOF\`
      3. Do the work
      4. Hand off (targets: reviewer, planner) → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom handoff --chatroom-id=test-chatroom-id --role=builder --next-role=<target> << 'EOF'
      ---MESSAGE---
      [Your message here]
      EOF\`
      5. Resume → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom get-next-task --chatroom-id=test-chatroom-id --role=builder\`
