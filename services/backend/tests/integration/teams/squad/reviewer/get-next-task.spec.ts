...
describe('Squad Team > Reviewer > Get Next Task', () => {
  test('task from user', () => {
    ...
    expect(output).toMatchInlineSnapshot(`
      ...
      2. (optional) Read context if needed → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom context read --chatroom-id="test-chatroom-id" --role="reviewer"\` _(skip if you already have full context)_
      3. Report progress at milestones → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom report-progress --chatroom-id="test-chatroom-id" --role="reviewer" << 'EOF'
      ---MESSAGE---
      [Your progress message here]
      EOF\`
      4. Do the work
      5. Hand off (targets: builder, planner) → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom handoff --chatroom-id="test-chatroom-id" --role="reviewer" --next-role=<target> << 'EOF'
      ---MESSAGE---
      [Your message here]
      EOF\`
      6. Resume → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom get-next-task --chatroom-id="test-chatroom-id" --role="reviewer"\`
      ...
      Context compacted? Run \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom get-system-prompt --chatroom-id="test-chatroom-id" --role="reviewer"\` to reload your role prompt.
      ============================================================"
    `);
  });
