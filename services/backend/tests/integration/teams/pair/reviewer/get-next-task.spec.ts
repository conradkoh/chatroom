...
describe('Pair Team > Reviewer > Get Next Task', () => {
  test('task from user', () => {
    ...
    expect(output).toMatchInlineSnapshot(`
      ...
      EOF\`
      4. Do the work
      5. Hand off (targets: builder, user) → \`CHATROOM_CONVEX_URL=http://127.0.0.1:3210 chatroom handoff --chatroom-id="test-chatroom-id" --role="reviewer" --next-role=<target> << 'EOF'
      ---MESSAGE---
      [Your message here]
      EOF\`
