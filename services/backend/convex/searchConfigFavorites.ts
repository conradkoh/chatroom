...
export const getSearchConfigFavorites = query({
  args: {
    ...SessionIdArg,
    machineId: v.string(),
  },
...
export const setSearchConfigFavorites = mutation({
  args: {
    ...SessionIdArg,
    machineId: v.string(),
    favorites: v.array(
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      await ctx.db.patch('chatroom_searchConfigFavorites', existing._id, data);
    } else {
      await ctx.db.insert('chatroom_searchConfigFavorites', data);
    }
