...
export const setPlannerRestartOnHandoffToUser = mutation({
  args: { ...SessionIdArg, chatroomId: v.id('chatroom_rooms'), role: v.string(), plannerRestartOnHandoffToUser: v.boolean() },
  handler: async (ctx, args) => {
    const auth = await getSession(ctx, args.sessionId);
    if (!auth) throw new ConvexError({ code: 'NOT_AUTHENTICATED', message: 'Authentication required' });
    const chatroom = await ctx.db.get('chatroom_rooms', args.chatroomId);
    if (!chatroom) throw new ConvexError({ code: 'CHATROOM_NOT_FOUND', message: 'Chatroom not found' });
    if (chatroom.ownerId !== auth.userId) throw new ConvexError({ code: 'UNAUTHORIZED', message: 'Not authorized to modify team agent configs for this chatroom' });
    if (!chatroom.teamId) throw new ConvexError({ code: 'CHATROOM_NO_TEAM_ID', message: 'Chatroom has no teamId — cannot build agent config key' });
    const teamRoleKey = buildTeamRoleKey(chatroom._id, chatroom.teamId, args.role);
    ...
    const now = Date.now();
    if (existing) await patchTeamAgentConfig(ctx, existing._id, { plannerRestartOnHandoffToUser: args.plannerRestartOnHandoffToUser });
    else {
      await deleteStaleTeamAgentConfigs(ctx, teamRoleKey);
      await ctx.db.insert('chatroom_teamAgentConfigs', { teamRoleKey, chatroomId: args.chatroomId, role: args.role, type: 'remote', plannerRestartOnHandoffToUser: args.plannerRestartOnHandoffToUser, createdAt: now, updatedAt: now });
    }
