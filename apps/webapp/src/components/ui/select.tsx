  ...
  function walk(children: React.ReactNode) {
    React.Children.forEach(children, (child) => {
      ...
      if (type?.[SELECT_ITEM_MARKER]) {
        ...
        if (value != null) items.push({ value, label: label ?? value });
        return;
      }
...
function Select<Value, Multiple extends boolean | undefined = false>({
  ...
  onOpenChange,
  ...props
}: Omit<SelectPrimitive.Root.Props<Value, Multiple>, 'onOpenChange' | 'items'> & {
  children?: React.ReactNode;
  ...
  const extractedItems = React.useMemo(() => extractSelectItems(children), [children]);

  return (
    <SelectPrimitive.Root
      data-slot="select"
      items={itemsProp ?? (extractedItems.length > 0 ? extractedItems : undefined)}
      open={open}
