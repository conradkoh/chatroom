...
export interface MobileKeyboardDrawerProps {
  open: boolean;
...
export function MobileKeyboardDrawer({
  open,
  onOpenChange,
  title,
  children,
  ...
  handleOnly = true,
  container,
}: MobileKeyboardDrawerProps) {
  const inset = useVisualViewportKeyboardInset(open);
  const offset = useVisualViewportOffsetTop(open);
  ...
  return (
    ...
    >
      <DrawerContent
        ...
        style={getMobileDrawerContentStyle(inset, offset)}
      >
        <DrawerHeader className={cn('shrink-0', headerClassName)}>
          {header ?? <DrawerTitle className="sr-only">{title}</DrawerTitle>}
        </DrawerHeader>
        <PickerShellProvider value={value}>
          <div className={cn('flex flex-col min-h-0 flex-1 overflow-y-auto', contentClassName)}>
            {children}
          </div>
        </PickerShellProvider>
      </DrawerContent>
