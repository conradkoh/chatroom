...
import {
  CHATROOM_SWITCHER_HEADING_ROW_HEIGHT,
  CHATROOM_SWITCHER_ITEM_ROW_HEIGHT,
  type ChatroomSwitcherRow,
...
export function ChatroomSwitcherVirtualizedList({
  ...
  const estimateSize = useCallback((_index: number, row: ChatroomSwitcherRow) => {
    if (row.type === 'heading') return CHATROOM_SWITCHER_HEADING_ROW_HEIGHT;
    return CHATROOM_SWITCHER_ITEM_ROW_HEIGHT;
  }, []);
      ...
      if (row.type === 'heading') {
        return (
          <div
            ...
            style={{ height: CHATROOM_SWITCHER_HEADING_ROW_HEIGHT }}
          >
            {row.label}
          </div>
        );
