...
export const ContentEditableInput = forwardRef<ContentEditableInputRef, ContentEditableInputProps>(
  function ContentEditableInput(
    ...
    // ── ZWS caret normalization ────────────────────────────────────────────
    // Zero-width spaces (\u200B) are inserted around file chips to enable
    // Alt+Arrow word-skip navigation. However, they create "phantom" caret
    // positions — the browser allows the caret to sit on either side of a
    // ZWS, which visually looks identical but confuses offset calculations
    // and trigger detection.
    //
    // This listener nudges the caret past any adjacent ZWS characters
    // whenever the selection changes, so the caret always rests on a
    ...
    useEffect(() => {
      ...
      const normalizeCaretAroundZWS = () => {
        // Guard against re-entry: setting the selection fires selectionchange again
        ...
        // Only normalize if the caret is inside our contenteditable
        if (!el.contains(anchorNode)) return;

        const text = anchorNode.textContent ?? '';
        if (!text.includes('\u200B')) return;

        // Find the nearest non-ZWS position by scanning forward then backward
        let newOffset = anchorOffset;

        // Nudge forward past any ZWS at or after the current position
        while (newOffset < text.length && text[newOffset] === '\u200B') {
          newOffset++;
        }

        // If we ended up at the end of the text node and it's all ZWS,
        // try nudging backward instead
        if (newOffset === text.length && newOffset > 0 && text[newOffset - 1] === '\u200B') {
      ...
      document.addEventListener('selectionchange', normalizeCaretAroundZWS);
      return () => {
        document.removeEventListener('selectionchange', normalizeCaretAroundZWS);
      };
    }, []);
