...
vi.mock('convex-helpers/react/sessions', () => ({
  useSessionMutation: (mutation: unknown) => mutation === 'agent' ? requestAgent : requestChatroom,
}));
...
describe('useAgentStop', () => {
  test('requests one agent aggregate stop', async () => {
    ...
    await act(() => result.current.requestAgentStop({ chatroomId: 'room' as never, machineId: 'machine', role: 'builder' }));
    expect(requestAgent).toHaveBeenCalledWith({ chatroomId: 'room', machineId: 'machine', role: 'builder', reason: 'user.stop' });
  });
