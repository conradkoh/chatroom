...
// Mock the two data hooks directly so each can return independent values
// without fighting over a shared useSessionQuery interceptor.
const mockUseSubscribeMessages = vi.fn();
const mockUseQueuedMessages = vi.fn().mockReturnValue([]);

vi.mock('./hooks/useSubscribeMessages', () => ({
  useSubscribeMessages: (...args: unknown[]) => mockUseSubscribeMessages(...args),
}));
...
describe('SessionMessageStream', () => {
  ...
  it('renders loading state when query returns undefined', () => {
    mockUseSubscribeMessages.mockReturnValue(undefined);
  ...
  it('renders empty placeholder when both streams are empty', () => {
    mockUseSubscribeMessages.mockReturnValue([]);
  ...
  it('shows queued messages with a Queued badge below the main stream', () => {
    mockUseSubscribeMessages.mockReturnValue([msg('m1', 'user', 'First', 1)]);
  ...
  it('does not show the queued zone when the queue is empty', () => {
    mockUseSubscribeMessages.mockReturnValue([msg('m1', 'user', 'Hello', 1)]);
    mockUseQueuedMessages.mockReturnValue([]);
    render(<SessionMessageStream sessionRowId={SESSION_ROW_ID} />);
