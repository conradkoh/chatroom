...
let mockCapabilities: { machineId: string | null; harnesses: unknown[] } = { machineId: null, harnesses: [] };
...
describe('SessionComposer', () => {
  ...
  it('renders textarea and send button when status is active', () => {
    render(<SessionComposer sessionRowId={SESSION_ROW_ID} status="active" workspaceId={SESSION_WORKSPACE_ID} harnessName={SESSION_HARNESS} />);
  ...
  it('renders status banner instead of input when status is closed', () => {
    render(<SessionComposer sessionRowId={SESSION_ROW_ID} status="closed" workspaceId={SESSION_WORKSPACE_ID} harnessName={SESSION_HARNESS} />);
