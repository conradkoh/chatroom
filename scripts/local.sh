...
# First-time setup: run pnpm setup if backend .env.local doesn't exist
if [ ! -f "services/backend/.env.local" ]; then
  echo -e "${YELLOW}⚙️  First-time setup detected. Running pnpm setup...${NC}"
...
# Create .chatroom and log directories
mkdir -p "$LOG_DIR"

# Kill any previously running processes
if [ -f "$PID_FILE" ]; then
  echo -e "${YELLOW}⚠️  Found existing PID file. Stopping previous processes...${NC}"
  bash "$SCRIPT_DIR/local-stop.sh"
  echo ""
fi

# Start backend (Convex dev with --local flag)
...
# Start machine daemon with the resolved backend URL
...
echo -e "${BOLD}${GREEN}   ✅ Local environment is running!     ${NC}"
echo -e "${BOLD}${GREEN}========================================${NC}"
echo ""
...
echo -e "   Update  : ${YELLOW}pnpm local:update${NC}"
echo ""
echo -e "${BOLD}Waiting for services to initialize... (check logs if something seems off)${NC}"
