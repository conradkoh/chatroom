...
export function useEventStreamFiltersFromUrl() {
  ...
  const setFilters = useCallback((v: EventStreamFilterValues) => {
    setState(v);
    replaceEventStreamFiltersInUrl(v);
  }, []);
  useEffect(() => {
    const onPopState = () => setState(readEventStreamFiltersFromSearch(window.location.search));
