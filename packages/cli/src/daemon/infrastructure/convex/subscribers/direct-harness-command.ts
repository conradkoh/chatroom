  ...
  const unsub = deps.wsClient.onUpdate(
    ...
    () => {
      ...
      void drain()
        .catch((err) =>
          console.warn(
            `[daemon] direct-harness-command drain error: ${err instanceof Error ? err.message : String(err)}`
          )
        )
        .finally(() => {
          processing = false;
    ...
    (err: unknown) => {
      console.warn(
        `[daemon] direct-harness-command subscriber error: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  );
