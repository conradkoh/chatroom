...
describe('user-message subscriber', () => {
  it('advances cursor only after async ingestion and skips non-user rows', async () => {
    ...
    const handle = startUserMessageSubscriber({ wsClient: ws.wsClient, sessionId: 's' as SessionId, machineId: 'm', chatroomId: 'room', loadUserIntentCursor: () => '0', saveUserIntentCursor: (_chatroomId, cursor) => save(cursor) }, onEvent);
  ...
  it('does not advance past a failed ingestion and stop waits for it', async () => {
    const ws = mockWs(); const save = vi.fn(); let resolve!: () => void; const pending = new Promise<void>((r) => { resolve = r; });
    const onEvent = vi.fn(() => pending); const handle = startUserMessageSubscriber({ wsClient: ws.wsClient, sessionId: 's' as SessionId, machineId: 'm', chatroomId: 'room', loadUserIntentCursor: () => '0', saveUserIntentCursor: (_chatroomId, cursor) => save(cursor) }, onEvent);
