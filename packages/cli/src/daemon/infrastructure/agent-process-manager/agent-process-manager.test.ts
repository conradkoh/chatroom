...
describe('AgentProcessManager', () => {
  ...
  describe('handleExit', () => {
    ...
    test('cursor-sdk spawn-error auth resumes existing session before cold restart', async () => {
      const cursorSdkService = {
        ...createMockService(),
        id: 'cursor-sdk',
        spawn: vi.fn().mockResolvedValue({
    ...
    test('cursor-sdk auth failure clears lastInFlightTask and awaits exit lifecycle before reopen', async () => {
      const cursorSdkService = {
        ...createMockService(),
        id: 'cursor-sdk',
        spawn: vi.fn().mockResolvedValue({
      ...
      manager.setLastInFlightTask(CHATROOM_ID, ROLE, 'task_1');
      const slot = manager.getSlot(CHATROOM_ID, ROLE)!;
      slot.recentLogLines = [
        '[cursor-sdk:planner@882x8x status] ERROR: Authentication error If you are logged in, try logging out and back in.',
      ...
      const enqueueSpy = vi
        .spyOn(deps.lifecycleOutbox, 'enqueue')
        .mockImplementation(async (fact) => {
          enqueueResolved = true;
          expect(fact.kind).toBe('exited');
          return { success: true };
        });
