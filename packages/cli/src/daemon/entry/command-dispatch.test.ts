 ...
 *   refreshModelsEffect, dispatchCommandEventEffect, createDaemonRuntime.
 *
 * refreshModelsEffect and dispatchCommandEventEffect use granular services
 * (DaemonSessionService / DaemonAgentProcessManagerService).
 * createDaemonRuntime uses granular services.
 *
...
vi.mock('./workspace-git/git-heartbeat.js', async () => {
...
describe('dispatchCommandEventEffect', () => {
  ...
  it('processes daemon.pickFolder and reports result to backend', async () => {
    const { dispatchCommandEventEffect } = await import('./command-dispatch.js');
    const { pickFolderDialog } = await import('../../infrastructure/local-actions/pick-folder.js');
    const deps = createMockDaemonDeps();
