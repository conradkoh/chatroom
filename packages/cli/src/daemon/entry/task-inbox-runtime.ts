...
> =>
  Effect.gen(function* () {
    ...
    const inboxPromise = runTaskInbox(
      {
        client: wsClient,
        sessionId: session.sessionId as import('convex-helpers/server/sessions').SessionId,
        machineId: session.machineId,
    ...
    void inboxPromise.catch((error) => {
      if (!stopped) console.warn('[TaskInbox] loop error:', error);
    });
    return {
      stop() {
        stopped = true;
