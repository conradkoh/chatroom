  ...
  const unsub = deps.wsClient.onUpdate(
    ...
    () => {
      ...
      void drain()
        .catch((err) =>
          console.warn(
            `[v2] direct-harness-prompt drain error: ${err instanceof Error ? err.message : String(err)}`
          )
        )
        .finally(() => {
          processing = false;
    ...
    (err: unknown) => {
      console.warn(
        `[v2] direct-harness-prompt subscriber error: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  );
