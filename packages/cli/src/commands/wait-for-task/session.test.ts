...
// Mock waitForTaskCommand to return a predictable string
vi.mock('@workspace/backend/prompts/base/cli/wait-for-task/command.js', () => ({
  waitForTaskCommand: (opts: { chatroomId: string; role: string; cliEnvPrefix: string }) =>
    `${opts.cliEnvPrefix}chatroom wait-for-task --chatroom-id=${opts.chatroomId} --role=${opts.role}`,
}));
...
describe('WaitForTaskSession', () => {
  ...
  describe('superseded response', () => {
    it('exits with code 0 and logs the event', async () => {
      ...
      expect(output).toContain('Another wait-for-task process started');
      expect(output).toContain('To reconnect, run:');
      expect(output).toContain('chatroom wait-for-task');
    });
  });
