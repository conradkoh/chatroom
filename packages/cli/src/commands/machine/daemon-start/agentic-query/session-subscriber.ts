...
async function getOrCreateHarness(
  daemonSession: AgenticQuerySubscriptionSession,
  deps: SubscriberDeps,
  session: AgenticPendingOpenSession,
  workspace: { workingDir: string }
): Promise<BoundHarness> {
  const harnessName = session.harnessName;
...
async function processOne(
  ...
  try {
    ...
    const modelConfig = session.lastUsedConfig.model;
    ...
    const idleConfig = {
      agent: session.lastUsedConfig.agent,
      model: session.lastUsedConfig.model,
    };
    ...
    console.log(
      `[agentic-query] Session opened: ${rowId} agent=${session.lastUsedConfig.agent} workspace=${session.workspaceId}`
    );
  } catch (err) {
    console.warn(
