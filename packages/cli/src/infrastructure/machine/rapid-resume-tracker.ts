...
export interface RapidResumeCheckResult {
  /** Whether the threshold was reached. */
  isStorm: boolean;
  /** Turn-end timestamps still inside the window. */
  recentEnds: number[];
  endCount: number;
  windowMs: number;
  threshold: number;
...
export class RapidResumeTracker {
  private readonly history = new Map<string, number[]>();

  /**
   * Record an agent_end and check whether auto-resume should abort.
   */
  record(chatroomId: string, role: string, now: number = Date.now()): RapidResumeCheckResult {
