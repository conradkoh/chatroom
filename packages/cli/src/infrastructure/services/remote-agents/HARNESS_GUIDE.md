...
- **pi**: Same daemon-memory model as opencode-sdk. `spawn` returns `harnessSessionId` from RPC `get_state` (sessions under `{workingDir}/.chatroom/pi-sessions`). `user.stop` with `preserveForResume` keeps session files on disk. The next start with `wantResume` calls `PiAgentService.resumeFromDaemonMemory` (`pi --session <id>`, same session-dir, prompt over stdin). Success emits `agent.sessionResumed`; failure emits `agent.sessionResumeFailed` and falls back to `spawn`. In-turn `resumeTurn` still handles agent_end without restart.
