...
describe('AgentProcessManager', () => {
  ...
  // ── Crash recovery bypasses concurrent limit ────────────────────────────

  describe('crash recovery bypasses concurrent limit', () => {
    test('platform.crash_recovery bypasses concurrent limit', async () => {
      const shouldAllowSpawn = deps.spawning.shouldAllowSpawn as ReturnType<typeof vi.fn>;
      shouldAllowSpawn.mockReturnValue({ allowed: true });
      ...
      expect(result.success).toBe(true);
      expect(shouldAllowSpawn).toHaveBeenCalledTimes(1);
      expect(shouldAllowSpawn).toHaveBeenNthCalledWith(
        1,
        CHATROOM_ID,
        'platform.crash_recovery',
        expect.objectContaining({ bypassConcurrentLimit: true })
      );
    });

    test('user.* reasons bypass concurrent limit', async () => {
      const shouldAllowSpawn = deps.spawning.shouldAllowSpawn as ReturnType<typeof vi.fn>;
      shouldAllowSpawn.mockReturnValue({ allowed: true });

      const result = await manager.ensureRunning(createOpts({ reason: 'user.restart' }));
    ...
    test('other reasons do NOT bypass concurrent limit', async () => {
      const shouldAllowSpawn = deps.spawning.shouldAllowSpawn as ReturnType<typeof vi.fn>;
      shouldAllowSpawn.mockReturnValue({ allowed: true });
      ...
      expect(shouldAllowSpawn).toHaveBeenNthCalledWith(
        ...
        CHATROOM_ID,
        'platform.auto_restart_on_new_context',
        expect.objectContaining({ bypassConcurrentLimit: false })
      );
    });
  });

  // ── Stop decrements concurrent count ────────────────────────────────────

  describe('stop decrements concurrent count', () => {
    test('recordExit is called when stop completes', async () => {
      await manager.ensureRunning(createOpts());
      ...
      expect(result).toEqual({ success: true });
      expect(deps.spawning.recordExit).toHaveBeenCalledWith(CHATROOM_ID);
    });
  });

  // ── handleExit emits agentStartFailed when restart is blocked ──────────

  describe('handleExit emits agentStartFailed when restart is blocked', () => {
    test('rate-limited restart emits agentStartFailed', async () => {
      const shouldAllowSpawn = deps.spawning.shouldAllowSpawn as ReturnType<typeof vi.fn>;
      shouldAllowSpawn.mockReturnValue({ allowed: true });

      await manager.ensureRunning(createOpts());

      // Now make the crash recovery restart be rate-limited
      shouldAllowSpawn.mockReturnValue({ allowed: false });

      // Simulate process exit — handleExit is async, must await
      ...
      // Should have called emitAgentStartFailed because restart was rate-limited
      // (mutation is called with api reference as first arg, args as second)
      const mutationMock = deps.backend.mutation as ReturnType<typeof vi.fn>;
      const allArgs = mutationMock.mock.calls.map((call: unknown[]) => call[1]);
      const hasRateLimitedError = allArgs.some(
        (arg: unknown) =>
          arg &&
          typeof arg === 'object' &&
          'error' in arg &&
          (arg as Record<string, unknown>).error === 'rate_limited'
      );
