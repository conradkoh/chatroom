...
import { DaemonAgentProcessManagerService, DaemonSessionService } from '../../daemon/entry/daemon-services.js';
import { onDaemonShutdownEffect } from './on-daemon-shutdown.js';

function runShutdown({ summaries, activeAgents }: { summaries: Record<string, { stoppedCount: number; failedCount: number }>; activeAgents: { chatroomId: string }[] }) {
  const executeScopedStopForCommand = vi.fn((args: { chatroomId: string }) => Effect.succeed(summaries[args.chatroomId] ?? { stoppedCount: 0, failedCount: 0 }));
  const agentPm = {
  ...
  const session = {
    ...
    backend: { mutation: vi.fn().mockResolvedValue({ stopCommandId: 'stop', inboxCommandId: 'inbox' }) },
  };
  return Effect.runPromise(onDaemonShutdownEffect.pipe(
...
describe('onDaemonShutdownEffect', () => {
  ...
  test('logs stopped and failed counts', async () => {
    ...
    await runShutdown({ activeAgents: [{ chatroomId: 'room-a' }, { chatroomId: 'room-b' }], summaries: {
      'room-a': { stoppedCount: 2, failedCount: 0 }, 'room-b': { stoppedCount: 1, failedCount: 2 },
    } });
  ...
  test('logs stopped count when all succeed', async () => {
    const log = vi.spyOn(console, 'log').mockImplementation(() => {});
    await runShutdown({ activeAgents: [{ chatroomId: 'room' }], summaries: { room: { stoppedCount: 2, failedCount: 0 } } });
